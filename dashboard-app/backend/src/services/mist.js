...
/**
 * Fetch an org-level network template by ID.
 *
 * @param {string} orgId
 * @param {string} templateId
 * @returns {object} - Template object from Mist API
 */
async function getOrgNetworkTemplate(orgId, templateId) {
  await validateToken();

  const url = `${BASE_URL}/api/v1/orgs/${orgId}/networktemplates/${templateId}`;
  console.log(`[Mist] GET ${url} (fetch network template)`);
  const res = await fetch(url, { headers: mistHeaders() });

  if (!res.ok) {
    const text = await res.text();
    throw new Error(`Mist get network template failed (${res.status}): ${text}`);
  }
...
/**
 * Merge new networks into an org-level network template.
 * Fetches current state, merges networks map, then PUTs the full object back.
 *
 * @param {string} orgId
 * @param {string} templateId
 * @param {object} networksMap  - { [networkName]: { vlan_id, subnet } }
 * @returns {object}            - Updated template from Mist API
 */
async function updateOrgNetworkTemplate(orgId, templateId, networksMap) {
  await validateToken();

  const url = `${BASE_URL}/api/v1/orgs/${orgId}/networktemplates/${templateId}`;

  // Fetch current state so we don't overwrite existing fields
  const getRes = await fetch(url, { headers: mistHeaders() });
  if (!getRes.ok) {
    const text = await getRes.text();
    throw new Error(`Failed to fetch template for update (${getRes.status}): ${text}`);
  }
  const current = await getRes.json();

  // Merge new networks into existing ones
  const merged = { ...current, networks: { ...(current.networks || {}), ...networksMap } };

  console.log(`[Mist] PUT ${url} (add ${Object.keys(networksMap).length} networks to template)`);
  ...
  if (!putRes.ok) {
    ...
    throw new Error(`Mist update network template failed (${putRes.status}): ${text}`);
  }
...
/**
 * Merge port profiles into an org-level network template's port_usages.
 * Fetches current template state, merges profilesMap, then PUTs it back.
 *
 * @param {string} orgId
 * @param {string} templateId
 * @param {object} profilesMap  - { [PROFILE_NNN]: { mode, port_network, voip_network } }
 */
async function updateOrgNetworkTemplatePortProfiles(orgId, templateId, profilesMap) {
  await validateToken();
  ...
  if (!getRes.ok) {
    const text = await getRes.text();
    throw new Error(`Failed to fetch template for port profile update (${getRes.status}): ${text}`);
  }
  const current = await getRes.json();

  const merged = {
    ...current,
    port_usages: { ...(current.port_usages || {}), ...profilesMap },
  };

  console.log(`[Mist] PUT ${url} (add ${Object.keys(profilesMap).length} port profiles to template)`);
  const putRes = await fetch(url, {
    method:  'PUT',
    headers: { ...mistHeaders(), 'Content-Type': 'application/json' },
    body:    JSON.stringify(merged),
  ...
  if (!putRes.ok) {
    const text = await putRes.text();
    throw new Error(`Mist update port profiles failed (${putRes.status}): ${text}`);
  }
